...
export function ListTableSwitch({ view, onChange, className }: ListTableSwitchProps) {
  ...
  return (
    <div
      className={cn(
        'inline-flex items-center gap-1 border border-black/15 rounded-full p-1 bg-white/60 backdrop-blur-sm',
        className
      )}
    >
      <button
      ...
      <button
        ...
        className={cn(
          'flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-mono transition-colors',
          view === 'table' ? 'bg-black text-white' : 'text-black/40 hover:text-black'
        )}
      >
        <Table2 size={13} />
        {t('common.table')}
