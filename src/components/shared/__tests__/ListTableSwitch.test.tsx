...
describe('ListTableSwitch', () => {
  it('renderiza os dois botões', () => {
    ...
    expect(screen.getByRole('button', { name: /common\.list/i })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /common\.table/i })).toBeInTheDocument()
  })

  it('chama onChange com "table" ao clicar em Tabela', async () => {
    const onChange = vi.fn()
    render(<ListTableSwitch view="list" onChange={onChange} />)
    await userEvent.click(screen.getByRole('button', { name: /common\.table/i }))
  ...
  it('chama onChange com "list" ao clicar em Lista', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: /common\.list/i }))
